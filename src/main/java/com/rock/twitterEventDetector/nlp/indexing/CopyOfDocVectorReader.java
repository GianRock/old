...
public class CopyOfDocVectorReader {
	private static class LazyHolder {
        private static final CopyOfDocVectorReader INSTANCE = new CopyOfDocVectorReader();
    }

    public static CopyOfDocVectorReader getInstance() {
	...
	private class GetRelativeDocFrequency implements Callable<Float>{
		...
		public Float call() throws Exception {
			...
			int tdf = getTermDocumentFrequencyInDayRange(term,
					"text", lowerDay,
					upperDay);
	 ...
	 * @throws Exception
	 */
	private Float getRelativeDocFrequency(String term,int lowerDay,int upperDay,int df) throws Exception{
		int tdf=0;
  		tdf = getTermDocumentFrequencyInDayRange(term,
					"text", lowerDay,
					upperDay);
	...
	/**
	 * dimensione di uno slot della fienestra temporale espressa in numero di giorni
	 * 
	 */
	...
	public CopyOfDocVectorReader() {

		try {
			Directory dir = FSDirectory.open(Paths
					.get(Constant.INDEX_DIR));
	...
	public CopyOfDocVectorReader(IndexWriter writer) {
				...
				if (termVector != null) {
					...
					while ((text = termsEnum.next()) != null) {
						...
						if (postingsEnum.nextDoc() != DocIdSetIterator.NO_MORE_DOCS) {
							...
							System.out.println(startOffset+ " + "+endOffset+"  "+text.utf8ToString());
							...
							try {
								...
								dft = getTermDocumentFrequencyInDayRange(text.utf8ToString(), "text", dayNumber,dayNumber+1);
								// docFreq = getTermDocumentFrequencyInDayRange(text.utf8ToString(), "text", 0,dayNumber+1);
							} catch (Exception e) {
								// TODO Auto-generated catch block
								e.printStackTrace();
							} 
					...
					// float timeBoost =calculateTimeBoost("text",text.utf8ToString(), dayNumber,documentsNumberPerDay);
						 float timeBoost=1f;
							...
							//System.out.println(text.utf8ToString()+ " TIME BOOOST " + timeBoost);
		...
		if (interval <= 1) {
			...
			try {
				float dfi = getTermDocumentFrequencyInDayRange(termText,
						fieldName, (interval) * DAY_TIME_INTERVAL_WIDTH + dayStart,
						(interval + 1) * DAY_TIME_INTERVAL_WIDTH + dayStart);

				int documentFreq = documentsNumberPerDay.get(documentsNumberPerDay.size()-1);
				float rdfi = (dfi + 1) / (documentFreq + 1);
				// rdfi=1.0f;
				List<Float> termFrequencies = new ArrayList<Float>();
			//	ExecutorService executor = Executors.newFixedThreadPool(interval);
				//List<Future<Float>> futureWeights=new ArrayList<Future<Float>>();
				...
				for (int i = 0; i < interval - 1; i++) {
					...
					termFrequencies.add(getRelativeDocFrequency(termText,(i)
							* DAY_TIME_INTERVAL_WIDTH + dayStart, (i+1)
							* DAY_TIME_INTERVAL_WIDTH + dayStart,documentsNumberPerDay.get(i)));
				}
				//executor.shutdown();
				/*
				for (Future<Float> future : futureWeights) {
					if(future.get()!=null)
						termFrequencies.add(future.get());
				}*/
	...
	public static void main(String[] args) {
		Long idTweet = 629867512667058176L;

 		MyTweet t1 = TweetsCollection.findTweetById(idTweet);
		System.out.println(t1.getCreateAt());
		CopyOfDocVectorReader doCReader = new CopyOfDocVectorReader();
		...
		MyPosTagger tagger=new MyPosTagger();
		TreeMap<Integer, TaggedWord> tmap=tagger.tagSentenenceToMap(t1.getCleanedText());
 
		for(Entry<Integer, TaggedWord> entry:tmap.entrySet()){
			System.out.println(entry.getKey()+" "+entry.getValue().tag() +" "+entry.getValue().word());
		}
	...
	public static void mains(String[] args) {
		...
		for (MyTweet t1 : tweets) {
			CopyOfDocVectorReader t = new CopyOfDocVectorReader();
				...
				if (termVector != null) {
					...
					while ((text = termsEnum.next()) != null) {
						...
						if (postingsEnum.nextDoc() != DocIdSetIterator.NO_MORE_DOCS) {
							...
							float tf = sim.tf(postingsEnum.freq());
							float timeBoost=1f;
						//	float timeBoost = calculateTimeBoost("text",text.utf8ToString(), dayNumber,);
